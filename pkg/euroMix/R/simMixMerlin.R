simMixMerlin <-
function(x,aa,afreq,generate=FALSE){
merlin(x, model=F, cleanup=F,generate.files=generate,options="--sim --save")
sim.ped <- scan(file = "merlin-replicate.ped", what = c(1, 1, 1, 1, 1, rep(c("", 1)), x$nMark*2), comment.char = "e", quiet = TRUE)
sim.ped <- sub("/", " ", sim.ped)
...
y=linkdat("_sim.ped",freq="merlin.freq",map="merlin.map",dat="merlin.dat",verbose=FALSE)
mixlist= lapply(y$markerdata, function(m) sort(unique(m[m>0])))
for (i in 1:length(mixlist))
mixlist[[i]]=aa[[i]][mixlist[[i]]]
id=x$available[1]
for (i in 1:x$nMark){
